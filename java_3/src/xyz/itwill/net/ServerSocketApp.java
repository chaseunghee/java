...
//서버(Server) : 외부의 컴퓨터(클라이언트)가 접속할 수 있는 환경(포트 개방)과 서비스를 제공하기 위한 컴퓨터
	...
	public static void main(String[] args) {
		for(int i=2000; i<=9000; i+=1000) {
			try {
				...
				//=> ServerSocket 클래스의 ServerSocket(int port)생성자를 이용하여 매개변수로 전달받은 
				//	포트번호를 활성화하여 ServerSocket 객체 생성
