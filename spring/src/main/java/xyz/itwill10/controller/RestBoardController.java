...
/* [순서-24]
	...
	REST 기능을 제공하는 요청 처리 메소드가 정상적으로 실행되는지 확인하기 위해 Advanced REST Client 크롬앱을 설치하여 사용 - Restful API 테스트 프로그램
...
public class RestBoardController {
	private final RestBoardService restBoardService; // restBoardService에 의존성 주입할 수 있도록 맨위에 @RequiredArgsConstructor
	
	/*
		페이지 번호를 전달받아 RESTBAORD 테이블에 저장된 게시글 중 페이지 번호에 출력될 게시글 목록을 검색하여 JSON 형식의 문자열로 응답하는 요청 처리 메소드
		
		REST 기능을 제공하는 요청 처리 메소드는 @RequestMapping 어노테이션 대신 @GetMapping,@PostMapping, @PutMapping, @PatchMapping, @DeleteMapping 어노테이션을 
		사용하여 요청 페이지를 요청 처리 메소드와 매핑 처리하는 것을 권장
		=> 요청방식 : GET(검색), POST(삽입), PUT(전체 변경), PATCH(부분 변경), DELETE(삭제) 등 @RequestMapping(value = "/board_list", method = RequestMethod.GET)
	*/
	@GetMapping("/rest/board_list")

	//Controller 클래스를 @RestController 어노테이션을 사용하여 Spring Bean으로 등록하였으므로 @ResponseBody 어노테이션을 생략해도 문자열로 응답 처리 가능
	//@ResponseBody
	...
	/* [순서-25]
		...
		=> [application/json] 형식의 문자열로 전달된 게시글을 Java 객체로 제공받아 매개변수에 저장하기 위해 @RequestBody 어노테이션 사용
	*/
	@PostMapping("/board_add")
	public String restBoardAdd(@RequestParam RestBoard restBoard) { //JSON 형식의 문자열을 전달받아 객체로 전달하고 싶을 때 사용 - @RequestBody
		/*
			HtmlUtils.htmlEscape(String str) : 매개변수로 전달받은 문자열에 저장된 HTML 태그 관련 문자를 회피문자(Escape Character)로 변환하여 반환하는 메소드 
			- XSS 공격에 대한 방어
		*/
	...
	*/
	...
	/* [순서-27] - 매개변수로 전달
		...
		요청 URL 주소로 값을  표현하여 전달하기 위해서는 @GetMapping 어노테이션의 value 속성값에 {이름} 형식으로 요청 URL 주소를 표현
		...
		=> 전달값의 이름과 매개변수의 이름이 다른 경우 @PathVariable 어노테이션에 value 속성을 사용하여 요청 URL 주소로 표현하여 전달된 값을 매개변수에 저장 가능
	*/
