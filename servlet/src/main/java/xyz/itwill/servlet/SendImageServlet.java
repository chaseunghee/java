	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		/*
			...
			=> 이미지 파일(이진 파일 : Binary File)를 생성하기 위해 원시데이터(1Byte)를 전달하기 위한 ServletOutputStream 객체 반환
		*/
		...
		*/
		...
		//System.out.println("imagleFilePath = "+imageFilepath); //콘솔창 결과 확인
		
		//매개변수에 이미지 파일의 파일 시스템 경로를 제공하여 파일 입력스트림 생성
		...
		*/
		while(true) {
			...
			if(readByte==-1) break; //공백이 입력되면 while문 종료
			out.write(readByte);
		}
