...
public class LogHelloWorldApp {
	/*
		...
		=> 로그 이벤트가 발생되면 로그 구현체가 로깅 정보를 제공받아 기록
		
		LoggerFactory 클래스 : Logger 객체를 생성하여 제공하기 위한 클래스
		LoggerFactory.getLogger(Class<T> clazz) : 메모리에 저장된 클래스(Class 객체 - Clazz)를 전달받아 해당 클래스에서 로그 이벤트를 발생할 수 있는
												Logger 객체를 생성하여 반환하는 메소드
	*/
