...
public class BeanAttributeApp {
	public static void main(String[] args) {
		...
		/*
			ApplicationContext.getBean(String beanName) : 스프링 컨테이너로부터 매개변수로 전달받은 beanName의 Spring Bean을 검색하여 반환하는 메소드
		...
		*/
		/*
			ApplicationContext.getBean(String beanName, Class<T> clazz) : 스프링 컨테이너로부터 매개변수로 전달받은 beanName의 Spring Bean(객체)를 Class 객체로 객체 형변환하여 반환하는 메소드
		*/
		...
		//bean.init(); //초기화 메소드
		bean.display();
		//bean 엘리먼트를 이용하면 메소드를 마무리 메소드를 객체 소멸 전 자동으로 호출되도록 설정 가능
		//bean.destroy(); //마무리 메소드
		System.out.println("=======================================================================");
		context.getBean("lazyInitBean",LazyInitBean.class);
		System.out.println("=======================================================================");
		ScopeBean bean1=context.getBean("singletonBean",ScopeBean.class); //getBean 처음에만 객체 생성함 
