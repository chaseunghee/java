...
/*
	*JDBC(Java DataBase Connectivity) : Java를 사용하여 DBMS 서버에 접속해 SQL 명령을 전달하여 실행하기 위한 기능을 제공하는 Java API(클래스 또는 인터페이스)
	...
	*java.sql : JDBC 기능의 프로그램을 작성하기 위한 클래스 및 인터페이스가 선언된 패키지
	 ...
	 => JDBC Driver가 포함된 라이브러리 파일(Jar 파일)을 다운로드 받아 프로젝트 빌드 처리해야만 라이브러리의 클래스를 사용하여 JDBC 프로그램 작성 가능 
	
	*Oracle DBMS를 이용한 JDBC 프로그램을 작성하기 위한 환경 설정
	-1.http://www.oracle.com 사이트에서 Oracle JDBC Driver 관련 라이브러리 파일을 다운로드
	...
	-3.프로젝트 폴더에 저장된 라이브러리 파일을 프로젝트에서 사용할 수 있도록 연결 - 빌드(Build) 처리
	 => 라이브러리 파일(Jar)의 믈래스 및 인터페이스를 프로젝트에서 사용 가능하도록 설정
	 => 프로젝트 >> 마우스 오른쪽 버튼 >> Properties(속성) >> Java Build Path(메뉴) >> Libraries(탭) >> classpath(선택) >> Add Jars(메뉴) >> 프로젝트 내부의 Jar 선택 >> Apply and Close
*/
...
public class InsertStudentApp {
	public static void main(String[] args) {
		...
		try {
			/*
				1.OracleDriver 클래스로 객체를 생성하여 DriverManger 클래스의 JDBC Drive 객체로 등록
					=> OracleDriver 클래스를 읽어 메모리에 저장하여 자동으로 OracleDriver 객체를 생성하여 DriverManager 클래스의 JDBC Driver 객체 등록
				*JDBC Drive 객체 : DriverManager 클래스에 등록되어 관리되는 Driver 객체 
				*Driver 객체 : DBMS 서버에 접속하여 SQL 명령을 전달하는 기능을 제공하는 객체
				*DriverManager 클래스 : Driver 객체를 관리하기 위한 기능을 제공하는 클래스
				*DriverManager.registerDriver(Driver diver) : Driver 객체를 매개변수로 전달받아 DriverManager 클래스가 관리할 수 있는 JDBC Driver 객체로 등록하는 메소드
				 => 동일한 클래스로 생성된 Driver 객체가 DriverManager 클래스에 여러 개 등록 가능
				 => 불필효한 Driver 객체가 존재하여 성능의 저하 발생
			 */
			//DriverManager.registerDriver(new OracleDriver());  - 권장X
			...
			*/
			...
			Class.forName("oracle.jdbc.driver.OracleDriver"); // catch (ClassNotFoundException e)해줘야 에러발생안함
			...
			*/ 
			 ...
			 *Statement 객체 : SQL 명령을 현재 접속중인 DBMS 서버에 전달하기 위한 기능을 제공하는 메소드
			*/
			
			stmt=con.createStatement();
			
			/*
			 4.Statement 객체를 사용하여 SQL 명령을 DBMS 서버에 전달하여 실행하고 실행결과를 반환받아 저장
			...
			*/
			...
			//String sql="insert into student values(1000,'홍길동','010-1234-5678','서울시 강남구','00/01/01')"; //실행 한 번만 가능, 두 번하면 에러발생 => 새로 다르게 작성하여 실행해야 삽입됨.
		...
		} finally { //예외발생과 상관없이 무조건 실행되는 명령을 작성하는 영역
			/*
