...
public class ConsoleIoApp {
	public static void main(String[] args) throws IOException {
		//1. new InputStreamReader(System.in); : InputStreamReader로 확장 => 1글자 문자데이터로받음
		//2. 아래와 같이 BufferReader로 더 확장 => 대량의 문자데이터로 받음
		//=> Java 프로그램 개발을 위해 기본적으로 제공되는 키보드 입력스트림(System.in)을 사용하여
		//	 InputStreamReader 클래스의 입력스트림으로 확장하고 확장된 Reader 객체를 전달받아
		//	 BufferedReader 클래스의 입력스트림으로 확장하여 대량의 문자데이터를 입력받을 수 있는 입력스트림 생서
		...
		//PrintStream클래스로 생성되어 print() 또는 println() 메소드로 호출하여 매개변수로 전달받은 값을
		//문자열로 변환하여 출력스트림에 전달해 출력 가능
		System.out.println("이름 입력 >> ");
		//BufferedReader.readLine() : 대량의 문자데이터를 읽어 문자열을 반환하는 메소드
