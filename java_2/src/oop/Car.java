...
//			필드(Field) : 표현 대상의 속성값을 저장하기 위한 변수 - 멤버변수
//			...
...
//=> 모든 메소드는 필드를 사용한 명령으로 필요한 기능을 제공하도록 제공
...
//public : 모든 패키지에 클래스에서 접근 가능하도록 허용하는 접근 제한자
...
public class Car {
	...
	//생성자(Constructor) : 생성자를 선언하지 않으면 매개변수가 없는 기본 생성자(Default Constructor)가
	//자동으로 생성되어 제공
	
	//메소드(Method) : 필드를 사용하여 메소드의 명령으로 필요한 기능을 제공하도록 작성
	...
	public void stopEngine(){//시동 OFF
		if(currentSpeed!=0) {//자동차가 멈춰있지 않은 상태인 경우  
			...
			*/
			...
			//=> 코드으 중복성을 최소화하여 프로그램의 생산성 및 유지보수의 효율성 증가
			speedZero();
		}
	...
	public void speedUp(int speed){//속도 증가
		...
		currentSpeed+=speed;
		System.out.println(modelName+"의 속도가 "+speed+"km/h 증가되었습니다. 현재속도는 "+currentSpeed+" km/h입니다.");
		//System.out.println("현재속도는 "+currentSpeed+"km/h입니다.");
	}
	...
	public void speedDown(int speed){//속도 감소
		...
		currentSpeed-=speed;
		System.out.println(modelName+"의 속도가 "+speed+"km/h 감소되었습니다. 현재속도는 "+currentSpeed+ "km/h입니다");
		//System.out.println("현재속도는 "+currentSpeed+"km/h입니다.");
	}
	...
	//=> 필드를 은닉화 처리하여 보호하여 메소드를 이용하여 처리되도록 설정
	//Getter메소드 : 클래스에 외부에서 필드값을 사용할 수 있도록 필드값을 반환하는 메소드
	...
	public void setModelName(String modelName) {
		...
		//=> this 키워드를 사용해서 필드 표현
		this.modelName=modelName;
	}


	//이클립스에서는 은닉화 처리된 필드에 대한 Getter메소드와 Setter메소드를 생성하는 기능 제공
	//단축키 : [Alt]+[Shit]+[S] >> 팝업메뉴 - [R] >> 필드 선택 >> Generate
