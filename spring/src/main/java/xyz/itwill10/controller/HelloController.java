...
//[순서-19] - 10.controller 패키지 생성 >> HelloController 클래스 생성 
...
/*
	...
	@Slf4j : @Controller 어노테이션을 사용하면 Controller 인터페이스를 상속받지 않아도 요청 처리 클래스로 작성 가능
	=> @RequestMapping 어노테이션을 사용하여 메소드를 요청 처리 메소드 처리되도록 작성
	=> @RequestMapping 어노테이션을 사용하여 요청 처리 메소드를 여러개 선언 가능 
*/

//[순서-21] - HelloController 코드 작성
...
public class HelloController {
	...
	*/
	...
	/* [순서-21]
		...
		=> 기본적으로 클라이언트의 모든 요청방식(Method - GET, POST, PUT, PATCH, DELETE 등)에 의해 호출되는 요청 처리 메소드를 작성할 경우 사용하는 어노테이션
		=> 클라이언트의 요청방식을 구분하여 요청 처리 메소드를 호출하고자 할 경우 @GetMapping, @PostMapping, @PutMapping, @PatchMapping, @DeleteMapping 등의 어노테이션을 사용 가능
