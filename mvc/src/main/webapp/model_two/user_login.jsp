...
<%-- 
	...
	=> [로그인] 태그를 클릭한 경우 [/login.do] 문서 요청 - 입력값 전달 
	
	로그인 상태의 사용자인 경우 - 환영메세지를 클라이언트에게 전달하여 응답하는 JSP 문서
	=> [회원목록] 태그를 클릭한 경우 [/list.do]  문서 요청
...
<%-- 요청 서블릿(컨트롤러)의 URL 주소 경로와 응답하는 JSP 문서의 경로가 서로 다르므로 웹자원의 경로는 절대경로로 표현하는 것을 권장  - 상대경로 비권장- 404오류 --%>
