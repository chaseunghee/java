...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%--
	회원정보를 전달받아 내장객체의 속성값으로 저장하고 [non_useBean_display.jsp] 문서로 포워드 이동하는 JSP 문서 - 클라이언트 요청에 대한 처리만 제공하는 JSP 문서
--%>
<%
	...
	/* ☆ - 위에부터 주석문이 한 세뚜
		JavaBean(객체)를 내장객체에 속성값으로 저장
		=> 동일한 클라이언트에서 사용되는 모든 JSP 문서에게 속성값 제공
	session.setAttribute("hewon", hewon);

		리다이렉트 이동 : 클라이언트에게 URL 주소를 전달하여 URL 주소의 다른 JSP 문서를 요청하여 실행결과를 응답받아 출력
		=> 클라이언트의 요청 URL 주소 변경 - 클라이언트를 이용하여 JSP 문서로 이동
	...
	*/
	...
	/*
		...
		=> 클라이언트의 요청 URL 주소 미응답 - 서버에서 JSP 문서 이동
		=> request 내장 객체에 속성값을 저장하여 포워드 이동되는 JSP 문서에서 반환받아 사용
		=> 포워드 이동되는 JSP 문서가 종료되면 request 내장객체에 저장된 속성값 자동 소멸
		
		forward 태그 대신 Java 명령을 사용하여 forward 이동 가능 (아래 주석 대신 아래 코드 사용)
		
		request.getRequestDispatcher(String url) : RequestDispatcher 객체를 반환하는 메소드
		=> RequestDispatcher 객체 : 스레드를 이동하기 위한 정보를 저장한 객체
		
		RequestDispatcher.forward(ServletRequest request, ServletResponse response) : 포워드 이동하는 메소드 - forward 태그와 동일
...
%>
<%-- <jsp:forward page="non_useBean_dispaly.jsp"/> --%>
