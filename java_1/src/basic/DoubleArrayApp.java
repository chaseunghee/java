...
public class DoubleArrayApp {
	public static void main(String[] args) {
		...
		//=> 행개수 : 2차원 배열에 생성되는 1차원 배열의 개수
		//=> 열개수 : 1차원 배열에 생성되는 요소의 개수
		...
		for(int i=0; i<num.length; i++) {//1차원 배열(행)을 일괄처리하기 위한 반복문
			for(int j=0; j<num[i].length; j++) {//요소(열)을 일괄처리하기 위한 반복문
		...
		//1차원 배열의 요소의 개수가 정해져있지 않느 2차원 배열 선언
		...
		int[][]array= {{10,20,30},{40,50},{60,740,80,90}};
		for(int[] arr : array) {
			for(int temp : arr) {
				System.out.print(temp+"\t");
			}
