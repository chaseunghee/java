...
/*
	클라이언트에게 쿠키를 전달하고 처리결과를 전달하여 웹문서로 응답하는 서블릿
	=> 서블릿에 의해 전달된 쿠키는 클라이언트에 저장
	
	쿠키(Cookie) : 서버(웹프로그램)와 클라이언트(브라우저)의 연결 지속성을 제공하기 위한 정보를 클라이언트에 저장하는 값(문자값)
	=> 클라이언트는 접속 서버의 정보를 식별자로 하여 쿠키 저장
	
	
*/

//실행 후 검사에 들어가서 application>>cookie>>http어쩌고 눌러서 확인 가능 
...
public class CookieCreateServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		Cookie countCookie=new Cookie("count", "0"); //숫자0은 안됨, new Cookie 생성자는 문자열로 전달받아야함.
		
		/*
			클라이언트에 전달되어 저장될 쿠키의 유지시간 변경
			
			Cookie.setMaxAge(int expiry) : 쿠키의 유지시간을 변경하는 메소드
			=> 매개변수에 쿠키를 유지하기 위한 시간(초)를 전달하면 클라이언트는 해당 시간동안 쿠키 유지
			
			setMaxAge() 메소드를 호출하지 않은 경우 쿠키의 유지시간에 대한 기본값은 [-1]로 설정
			=> 유지시간이 [-1]로 설정된 쿠키는 브라우저 종료시 자동 소멸
		*/
		countCookie.setMaxAge(24*60*60); //쿠키의 유지시간을 24*60*60(1일)로 변경
		...
		/*
			...
			HttpServletResponse.addCookie(Cookie cookie) : 클라이언트에게 Cookie 객체를 전달하는 메소드 (reponse메세지에 cookie 추가한다는 메소드)
		*/
