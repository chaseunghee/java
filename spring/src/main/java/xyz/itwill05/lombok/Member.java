...
/*
	클래스 작성에 필요한 메소드를 자동으로 생성해주는 Lombok 라이브러리를 사용하는 방법
	1.Lombok 라이브러리를 프로젝트 빌드 처리 - 메이븐 사용 : pom.xml
	2.이클립스(IDE)를 종료하고 콘솔(명령 프롬프트 - cmd)를 관리자 권한으로 실행한 후 Lombok 라이브러리가 저장된 로컬 저장소(Local Repository 
		- 사용자 폴더의 .m2 폴더)의 라이브러리 폴더로 이동
	3.콘솔에서 Jar 프로그램 실행하여 Lombok 라이브러리를 이클립스에 사용할 수 있도록 설정
	=> Jar 프로그램 실행 방법 : java -jar lombok-1.18.28.jar
	=> 탐색기에서 로컬 저장소의 라이브러리 폴더로 이동하여 Jar 파일을 더블클릭하여 실행 가능
	4.Jar 프로그램을 실행하여 제공된 설치창(Installer)에서 Lombok 라이브러리를 사용할 이클립스를 선택하여 Lombok 설치 
...
/*
	...
	=> 필드에 @Setter 어노테이션을 사용하면 해당 필드에 대한 Setter 메소드를 자동 생성하여 제공
	
	@Getter : 클래스에 선언된 모든 필드에 대한 Setter 메소드만을 자동 생성하여 제공하는 어노테이션
	...
	@Data : 기본 생성자, Setter 메소드, toString() 메소드, equals()메소드, hashCode() 메소드를 자동 생성하여 제공하는 어노테이션 
	...
	=> 매개변수가 선언된 생성자에 @Builder 어노테에션을 사용하면 생성자 매개변수로 초기화 처리하기 위한 필드에만 값을 저장할 수 있는 기능 제공
