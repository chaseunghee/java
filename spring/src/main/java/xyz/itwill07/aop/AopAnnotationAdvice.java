...
/*
	@Component : Compoent 어노테이션으로 Spring Bean 등록
...
public class AopAnnotationAdvice {
	...
	*/
	...
	/* [순서-8]
		...
		@AfterReturning : 타겟메소드의 핵심관심코드가 정상적으로 실행 후에 삽입되어 실행될 횡단관심코드를 제공하기 위한 어노테이션
		=> Spring Bean Configuration File의 after-returning 엘리먼트와 유사한 기능 제공
		
	@AfterReturning("aopPointCut()")
	public void afterReturningLog() {
		log.info("[afterReturningLog]핵심관심코드 정상적으로 실행 후에 삽입되어 실행될 횡단관심코드");
	}
	 */
	...
	/* [순서-9]
		...
		returning 속성 : 타겟메소드의 반환값을 전달받아 저장하기 위한 매개변수의 이름을 속성값으로 
	*/
	@AfterReturning(value="aopPointCut()", returning="object") 
	public void afterReturningLog(Object object) { //타겟메소드의 반환값 object
		log.info("[afterReturningLog]핵심관심코드 정상적으로 실행 후에 삽입되어 실행될 횡단관심코드");
	}
	...
	}//app에서 실행하면 예외가 안떨어지기 때문에 afterthrowing 발생x => 예외가 떨어지게 강제로 AopAnnotaitonBean에 코드 작성
	 */
