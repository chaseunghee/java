...
public class PointBoardServiceImpl implements PointBoardService{
	...
	/*
		매개변수로 게시글을 전달받아 POINTBOARD 테이블에 게시글로 삽입하고 게시글 작성자에 대한 회원정보를 POINTUSER 테이블에서 검색하여 반환하는 메소드
		=> POINTUSER 테이블에 저장된 회원정보 중 게시글 작성자에 대한 회원정보의 포인트를 증가되도록 변경 처리
	*/
	/* [순서-24] - addPointBoard(), removePointBoard() 메소드 위에 @Transactional 어노테이션 작성
	 			 - getPointBoardList() 메소드는 select라 어노테이션 작성안함 : select가 검색기능만 있어서 그런듯
	...
	*/
	...
	/*
		매개변수로 글번호을 전달받아 POINTBOARD 테이블에 저장된 게시글을 삭제하고 게시글 작성자에 대한 회원정보를 POINTUSER 테이블에서 검색하여 반환하는 메소드
