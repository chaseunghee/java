...
public class NotepadApp extends JFrame {
	...
	JTextArea jTextArea;
	JMenuItem init, open, save, exit; // 초기화, 열기, 저장, 끝
	...
	public NotepadApp(String title) {
		...
		save.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S, InputEvent.ALT_DOWN_MASK));
		...
		setJMenuBar(jMenuBar);//jMenuBar라는 JMenuBar 생성
		
		jTextArea=new JTextArea();
		jTextArea.setFont(new Font("굴림체",Font.PLAIN,20));
		JScrollPane jScrollPane=new JScrollPane(jTextArea);
		
		getContentPane().add(jScrollPane,BorderLayout.CENTER); //ContentPane의 setting 할 때 getContentPane()호출하여 사용
		...
		setDefaultCloseOperation(EXIT_ON_CLOSE);//윈도우창 종료시 프로세스까지 닫음
		setBounds(450,150,1000,600);//배치관리자로 (가로위치, 세로위치, 가로길이, 세로길이)
		setVisible(true);//true: 창 보여주기
	}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(eventSource==init) {
				...
				try {
					...
					JOptionPane.showMessageDialog(null, "프로그램의 문제가 발생했습니다.");
				}
				...
				try {
					...
					out.write(text); //문자열 통째로 가능-write
					
					out.close(); 

				}catch (IOException exception) { //파일을 만들어줄거라서 FileNotFoundException은 안해도됨.
					JOptionPane.showMessageDialog(null, "프로그램의 문제가 발생했습니다.");
				}
