...
public class InetAddressApp {
	public static void main(String[] args) throws UnknownHostException {
		...
		//InetAddress.getLocalHost() : 현재 사용중인 컴퓨터의 네트워크 식별자가 지정된 InetAddress 객체를 반환하는 메소드
		//=> UnknownHostException 발생(일반예외) : 네트워크에서 사용하는 호스트이름의 컴퓨터를 검색할 수 없는 경우 발생
		//현재 사용중인 컴퓨터는 기본적으로 [127.0.0.1]의 IP 주소가 제공되며
		//[localhost]라는 이름으로 호스트 이름(네트워크에서 사용하는 컴퓨터 이름) 설정
		...
		System.out.println("myCoumputer = "+myComputer.getHostName());

		//InetAddress.getHostAddress() : InetAddress 객체에 저장된 네트워크 식별자에서 
		//							IP 주소를 문자열로 변환하여 반환하는 메소드
		System.out.println("myCoumputer = "+myComputer.getHostAddress());
