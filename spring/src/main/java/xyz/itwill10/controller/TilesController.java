...
/* [순서-25]
	...
	TilesView 클래스 : 요청 처리 메소드의 반환값(ViewName)을 제공받아 다수의 JSP 문서가 결합된 JSP 문서(템플릿 페이지)로 응답하기 위한 기능을 제공하는 클래스 - ViewResolver
	...
	3.Front Controller(DispatcherServlet 클래스)가 요청 처리 메소드의 반환값(ViewName)을 제공받아 TilesView 클래스를 이용하여 응답 처리되도록
	 	Spring Bean Configuration File(servlet-context.xml) 설정
