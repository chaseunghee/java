...
/*
	...
	=> 세션을 구분하기 위한 식별자(SessionId)를 이용하여 클라이언트를 구분해 세션에 값(객체)를 저장하여 사용
	
	세션 바인딩(Session Binding) : 연결 지속성을 제공하기 위한 값(객체)이 저장된 세션을 웹프로그램에서 사용할 수 있도록 결합하는 작업 - WAS 
	
	클라이언트에서 [JSESSIONID] 이름의 쿠키값을 제공받지 못한 경우 메모리에 새로운 세션을 생성하여 바인딩 처리하고 생성된 세션의 식별자(SessionId)를 클라이언트에게 [JSESSIONID] 이름의 쿠키값으로 저장되도록 전달
	...
	세션 트랙킹(Session Tracking) : 클라이언트에게 제공받은 [JSESSIONID] 이름의 쿠키값과 메모리에 저장된 세션의 식별자(SessionId)를 비교하여 검색하는 작업
*/
...
public class SessionServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		HttpSession session=request.getSession(); //메소드만 호출하면 was가 알아서 해줌
		...
		out.println("<hr>");
		/*
			HttpSession.isNew() : 세션을 트랙킹하여 바인딩된 경우 [false] 반환
								  세션을 생성하여 바인딩돈 경우 [true] 반환
		*/
		if(session.isNew()) {
		...
		*/
		...
		/*
			HttpSession.getMaxInactInterval() : 세션 유지시간(초)를 반환하는 메소드
			=> 세션 유지시간 : 세션을 사용하지 않을 경우 소멸되도록 설정하는 시간
		*/
		out.println("<p>세션 유지시간 = "+session.getMaxInactiveInterval()+"</p>");
		
		/*
			HttpSession.setAttribute(String atributeName, Object attributeValue) 
			...
			동일한 세션을 바인딩한 모든 웹프로그램에게 속성값을 제공하여 사용 가능
		...
		*/
		...
		/*
			HttpSession.getAttribute(String attributeName) : HttpSession 객체에 저장된 속성값을 반환하는 메소드 - 매개변수에 속성값을 구분하기 위한 속성명 전달
			=> HttpSession 객체에 저장된 속성값을 Object 객체로 반환하므로 반드시 명시적 객체 형변환
		...
		*/
		...
		/*
			HttpSession.removeAttribute(String attributeName) : HttpSession 객체에 저장된 속성값을 삭제하는 메소드 - 매개변수에 속성값을 구분하기 위한 속성명 전달
		*/
		session.removeAttribute("now");
		
		/*
			HttpSession.invalidate() : 바인딩된 세션을 언바인딩하여 무효화 처리하는 메소드 
