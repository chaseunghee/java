...
public class StudentGUIApp extends JFrame implements ActionListener {
	...
	//이벤트에 따른 JTextField 필드의 활성화 상태 변경
	...
	//이벤트에 따른 JTextField 컴퍼넌트와 JButton 컴퍼넌트 활성화 상태 변경
	...
	public void displayAllStudent() {
		...
		List<StudentDTO> stuList=StudentDAOImpl.getDAO().selectAllStudentList();
		...
		/*
			JTable.getModet() : JTable 컴퍼넌트의 행정보가 저장된 TableModel 객체를 반환하는 메소드
			 => DefaultTableModel 클래스로 명시적 객체 형변환
			DefaultTableModel 객체 : JTable 컴퍼넌트의 행과 열을 조작할 수 있는 메소드를 제공하는 객체 
		*/
		DefaultTableModel model=(DefaultTableModel)table.getModel(); //형변환하지 않으면 에러뜸 table.getModel() 인터페이스 -> 클래스 (DefaultTableModel)table.getModel()
		
		
		//JTable 컴퍼넌트 초기화 - 기존 출력행 삭제 처리
		for(int i=model.getRowCount(); i>0; i--) { //JTable 컴퍼넌트의 첫 번째 행 제거 (기존에 있던 행 싹 사라짐)
			model.removeRow(0);
		}
		
		//List 객체의 요소(StudentDTO 객체)를 하나씩 제공받아 반복 처리
		for(StudentDTO student : stuList) { //검색된 학생정보를 Vector로 만들어줌
	...
	//JTextField 컴퍼넌트로 입력된 학생정보를 제공받아 STUDENT 테이블에 삽입하고 STUDENT 테이블에 저장된 모든 학생정보를 검색하여 JTable 컴퍼넌트에 출력하는 메소드
	...
	public void addStudent() {
		...
		if(noTemp.equals("")) {
			...
			//JTextField 컴퍼넌트에 입력 초점을 제공하는 메소드
			noTF.requestFocus();
			return;
		}
		
		//1-2.정규표현식과 입력값의 입력패턴이 다른 경우
		String noReg="^[1-9][0-9]{3}$";
		
		if(!Pattern.matches(noReg, noTemp)) {
			JOptionPane.showMessageDialog(this, "학번은 4자리 숫자로만 입력해주세요.");
		...
		if(StudentDAOImpl.getDAO().selectStudent(no)!=null) {
			JOptionPane.showMessageDialog(this, "이미 사용중인 학번을 입력하였습니다.");
		...
		if(phone.equals("")) {
			JOptionPane.showMessageDialog(this, "전화번호를 반드시 입력해주세요.");
		...
		if(address.equals("")) {
			JOptionPane.showMessageDialog(this, "주소를 반드시 입력해주세요.");
		...
		if(birthday.equals("")) {
			JOptionPane.showMessageDialog(this, "생년월일을 반드시 입력해주세요.");
		}
	...
	//JTextField 컴퍼넌트로 입력된 학생정보를 제공받아 STUDENT 테이블에 저장된 학생정보를 변경하고 STUDENT 테이블에 저장된 모든 학생정보를 검색하여 JTable 컴퍼넌트에 출력하는 메소드
	...
	//JTextField 컴퍼넌트로 입력된 학번을 제공받아 STUDENT 테이블에 저장된 해당 학번의 학생정보를 삭제하고 STUDENT 테이블에 저장된 모든 학생정보를 검색하여 JTable 컴퍼넌트에 출력하는 메소드
...
String name=nameTF.getText();
		...
		//이름을 전달받아 STUDENT 테이블에 저장된 해당 이름의 학생정보를 검색하여 반환하는 DAO 클래스의 메소드 호출
