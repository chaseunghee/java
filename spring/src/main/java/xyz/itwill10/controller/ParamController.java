...
//[순서-10] - ParamController 클래스 생성

@Controller
public class ParamController {
	@RequestMapping(value="/param", method=RequestMethod.GET)
	public String form() {
		return "param_form"; //=>[순서-11] - param_form.jsp 파일 생성
	}
	
	/*[순서-12]
		
		요청 처리 메소드의 매개변수에 HttpServletRequest 인터페이스로 매개변수를 선언
		=> HttpServletRequest 객체의 메소드를 호출하여 현재 페이지 요청시 전달된
	...
	public String result(HttpServletRequest request) throws UnsupportedEncodingException {
		...
		return "param_display"; //=>[순서-13] -param_display.jsp 파일 생성
	}
	 */
	
	/* [순서-14]
	
		요청 처리 메소드에서 현재 페이지를 요청할 때 전달된 값의 이름과 같은 이름으로 원시형 또는 String 클래스의 매개변수를 선언하면 Front Controller는 전달값을 
		매개변수에 자동으로 저장하여 제공
		=> 전달값의 이름과 매개변수의 이름이 같지 않은 경우 String 자료형의 매개변수에는 [null]저장
		=> 원시형의 매개변수는 전달값의 이름과 매개변수의 이름이 같지 않은 경우 또는 원시형에 맞지 않은 값이 전달된 경우 400 에러코드 발생
		
		매개변수를 이용하여 전달값을 제공받아 사용하기 전에 리퀘스트 메세지 몸체부에 저장되어 전달되는 값에 대한 문자형태(캐릭터셋) 변경 처리 - 인코딩 필드 사용
		...
		클라이언트의 모든 요청에 대해 리퀘스트 메세지 몸체부에 저장되어 전달되는 값에 대한 문자형태(캐릭터셋)을 변경하는 인코딩 필터가 사용되도록 [web.xml] 파일 설정
	@RequestMapping(value = "/param", method = RequestMethod.POST)
	...
	*/
	...
	/* [순서-17]
		...
		=> @RequestParam 어노테이션으로 전달값이 매개변수에 저장되도록 매개변수의 이름과 전달값
		=> required 속성값을 [true]로 설정한 경우 전달값이 없으며 매개변수에 [null] 저장 - 400 에러코드 미발생
	...
	*/
	...
	/* [순서-19]
		...
		defaultValue 속성 : 전달값과 매개변수의 이름이 다른 경우 매개변수에 저장된 기본값을 속성
