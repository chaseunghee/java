...
/*
	javax.sql.DataSource : DBCP 클래스를 작성하기 위해 상속받기 위한 인터페이스
		=> DBCP 클래스의 메소드가 동일한 형식으로 작성하기 위한 규칙 제공
*/
/*
...
*/ 
//빨간색 - 클래스 , 보라색 - 인터페이스, 팩토리 클래스가 전달받아서 pooldatasource데이터소스를 만들어줄 때 미리 컨넥션을 만드러줌
public class DataSourceApp {
	public static void main(String[] args) throws SQLException {
		/*
			...
			=>PoolDataSource 객체를 생성하여 반환하는 메소드
		*/
		PoolDataSource pds=PoolDataSourceFactory.getPoolDataSource(); //PoolDataSourceFactory한테 PoolDataSource 객체를 만들어 주세요를 의미
		
		//아래 4개 : PoolDataSource 객체에 저장될 Connection 객체를 생성하기 위한 메소드 호출 
		/*
		...
		*/
		...
		/*
			PoolDataSource 객체에 저장될 Connection 객체의 개수를 제한하기 위한 메소드 호출
			=> 메소드 호출을 생략할 경우 기본값을 사용하며 Connection 개수 제한
		*/
		pds.setInitialPoolSize(2); //최초 생성되는 Connection 객체의 개수 변경
		pds.setMaxPoolSize(3); //최대 생성되는 Connection 객체의 개수 변경
		...
		*/
		...
		//빌려다 쓸 때는 getConnection . 다 썼으면 close만 해줘도 알아서 돌아감, close 안해주면 기본 2개 최대3개까지 만들어짐.
