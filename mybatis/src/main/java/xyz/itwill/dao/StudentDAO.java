	...
	*/
		...
		try {
			/*
				...
				Resources.getResourceAsStream(String resource) : 매개변수에 mybatis 환경설정파일 경로를 전달받아 파일 입력스트림을 생성하여 반환하는 메소드
			*/
		...
		/*
			SqlSessionFactoryBuilder.build(InputStream inputStream) : 매개변수로 mybatis 환경설정파일의 입력스트림을 전달받아 SqlSessionFactory 객체를 생성하여 반환하는 메소드
		*/
	...
	*/
		public List<Student> selectStudentList() {
			/*	
				...
				=> SqlSession 객체 : 매퍼에 등록된 정보를 이용하여 SQL 명령을 제공받고 실행하고 실행결과를 Java 객체로 매핑 처리하여 반환하는 기능을 제공하는 객체
			*/
			...
			try {
				/*
					...
					select 엘리먼트의 식별자(id 속성값)을 이용한 문자열을 전달받아 매퍼에 등록된 SQL 명령을 DBMS 서버에 전달하여 실행하고 실행결과를 Java 객체로 매핑 처리하여 반환
				*/
