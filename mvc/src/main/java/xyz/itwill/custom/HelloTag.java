...
public class HelloTag extends TagSupport {
	...
	//JSP 문서에서 커스텀 태그를 최초로 사용할 경우 태그 클래스를 객체로 생성하기 위해 한 번만 호출 - was가 HelloTag 클래스를 객체로 생성해줌(커스텀 태그를 최초로 사용할 경우) 
	public HelloTag() {
		System.out.println("HelloTag 클래스의 기본 생성자 호출 - 객체 생성");
	}
	
	
	//doStartTag Override - JSP 문서에서 커스텀 태그의 시작태그를 사용할 때마다 호출되는 메소드
	@Override
	public int doStartTag() throws JspException {
		System.out.println("HelloTag 클래스의 doStartTag() 메소드 호출");
		try {
			/*
				부모클래스로부터 제공받은 PageContext 객체(pageContext)를 이용하여 웹프로그램 작성에 필요한 객체를 반환받아 명령 작성
	...
	//doAfterBody Override - JSP 문서에서 커스텀 태그의 태그내용을 사용할 때마다 호출되는 메소드
	@Override
	...
	//doEndTag Override - JSP 문서에서 커스텀 태그의 종료태그를 사용할 때마다 호출되는 메소드
	@Override
