...
<body>
	...
	<%-- Header 영역 
	<h1>쇼핑페이지</h1>
	<a href="index.jsp">쇼핑(Shopping)</a>&nbsp;&nbsp;
	<a href="index.jsp">블로그(Blog)</a>&nbsp;&nbsp;
	<a href="index.jsp">블로그(Cafe)</a>&nbsp;&nbsp;
	<hr>
	--%>
	...
	<%-- 
		...
		=> JSP 문서를 요청한 경우 include Directive file 속성값으로 설정된 외부파일의 소스코드를 JSP 문서에 포함한 후 실행한 결과를 클라이언트에게 전달하여 응답
		=> include Directive의 file 속성값으로 설정된 외부파일의 내용이 변경될 경우 JSP 문서를 변경한 것과 동일하므로 JSP 문서를 다시 해석하여 서블릿으로 생성
	...
	<%--
		...
		=> JSP 문서의 실행결과를 CSL(HTML,CSS,JavaScript)로 가져와 포함
	...
	<%--
		...
		요청 JSP 문서에서 다른 JSP 문서로 스레드를 이동할 경우 요청 JSP 문서의 request 객체와 response 객체를 스레드가 이동되는 JSP 문서로 전달하여 사용
		=> param 태그를 사용하면 request 객체의 몸체부에 값을 저장하면 전달 
		
		include 태그와 forward 태그의 하위태그로 param 태그를 제외한 문장이 존재할 경우 에러 발생 - JSP 주석 제외 
	--%>
