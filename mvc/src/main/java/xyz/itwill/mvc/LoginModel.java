...
/*
	...
	=> 로그인정보를 전달받아 USERINFO 테이블에 저장된 회원정보와 비교하여 인증 처리한 후 인증 성공시 세션에 권한 관련 객체를 속성값으로 저장하고 
		[/loginform.do] 페이지로 리다이렉트 이동하기 위한 정보가 저장된 ActionForward 객체 반환
	=> 인증 실패시 [user_login.jsp] 문서로 포워드 이동하기 위한 정보가 저장된  ActionForward 객체 반환 - 에러메세지와 아이디를 request 객체의 속성값으로 저장하여 JSP 문서에 제공
*/
public class LoginModel implements Action{ //인터페이스 Action 상속(구현)
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			*/
			...
			//모델 클래스의 요청 처리 매소드에서는 Service 클래스의 객체로 메소드를 호출하여 데이타 처리 관련 기능이 실행되도록 작성
