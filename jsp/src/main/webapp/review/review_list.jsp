...
<%-- 
	...
	=> 게시글을 페이지로 구분하여 검색 - 페이징 처리 - startRow, endRow 필요
	=> [페이지 번호] 태그를 클릭한 경우 [review/review_list.jsp] 문서 요청 - 페이지번호,검색대상,검색단어 전달
	=> [검색] 태그를 클릭한 경우 [review/review_list.jsp] 문서 요청 - 검색대상,검색단어 전달
	=> [글쓰기] 태그를 클릭한 경우 [review/review_write.jsp] 문서 요청 - 로그인 상태의 사용자에게만 링크 제공
	=> 글쓰기의 [제목] 태그를 클릭한 경우 [review/review_detail.jsp] 문서 요청 - 글번호, 페이지번호, 검색대상, 검색단어 전달
--%> 

<%
 /*
 		게시글 검색 기능에 필요한 전달값(검색대상과 검색단어)을 반환받아 저장
 		- search와 keyword가 null이면 게시글 검색기능을 사용하지 않음을 의미(ReviewDAO의 selectReviewCount 메소드의 if문)
 	*/
 	...
 	/*
 		페이징 처리에 필요한 전달값(요청 페이지 번호)을 반환받아 저장
 		=> 페이징 처리에 필요한 전달값이 없는 경우 1번째 페이지의 게시글 목록을 검색하여 응답 처리
 	*/
 	int pageNum=1; 
 	...
 	//하나의 페이지에 검색되어 출력될 게시글의 개수 설정 - 전달값으로 반환받아 저장 가능
 	int pageSize=10;
 	
 	/*
 		검색 관련 정보를 전달받아 REIVEW 테이블에 저장된 게시글 중 검색 처리된 전체 게시글의 개수를 검색하여 반환하는 DAO 클래스의 메소드 호출
 		=> 게시글 검색 기능 미구현시 호출되는 메소드
 	*/
 	...
 	*/
 	...
 	//페이징 처리 관련 정보와 게시글 검색 기능 관련 정보를 전달하여 REVIEW 테이블에 저장된 게시글 목록을 검색하여 List 객체로 반환하는 DAO 클래스의 메소드 호출  
 	...
 	*/
 	...
 	/*
 		페이지에 출력될 게시글 일련번호 시작값을 계산하여 저장 - ReviewDTO의 num은 식별자 역할만 하고 게시글 일련번호로 사용 못하므로 아래 코드(시작값) 작성
 		=> 검색 게시글의 개수 : 91 >> 1Page : 91~82, 2Page : 81~72, 3Page : 71~62, ... 
 	*/
 	int printNum=totalReview-(pageNum-1)*pageSize;//글번호 출력하기 위한 코드
 %>
<style type="text/css">
#review_list {
	width: 1000px;
...
<h1>제품후기 목록</h1>
<div id="review_list">
	<div id="review_title">ReviewList(<%=totalReview %>)</div>
	
	<%if(loginMember!=null){ //로그인 상태의사용자인 경우 %>
	...
	<%if(totalReview==0) {//전체 리뷰수가 없을 경우%>
	<tr>
		<td colspan="5">검색된 게시글이 없습니다.</td>
	</tr>
	<%} else {%>
		<%-- List 객체의 요소(ReviewDTO 객체)를 하나씩 제공받아 반복처리 --%>
		...
		<tr>
			...
			<%if(review.getStatus()!=0){ //삭제 게시글이 아닌 경우 - 작성자, 조회수, 작성일자 출력 %>
			<%-- 작성자 --%> 
			...
			<%-- 작성일자 : 오늘 작성된 게시글은 시간만 출력하고 오늘 작성일이 아닌 게시글인 경우 날짜와 --%>
			<td>
	...
	<%
		//하나의 페이지 블록에 출력될 페이지 번호의 개수를 설정
		int blockSize=5;
