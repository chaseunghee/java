...
public class ByteStreamApp {
	public static void main(String[] args) throws IOException {
		...
		while(true) {
			...
			//InputStream.read() : 입력스트림에 존재하는 값을 원시데이터(1Byte)로 읽어 정수값(int)으로 
			//						반환하는 메소드
			//=> 입력스트림에 값이 없는 경우 스레드는 일시 중지 상태로 전환
			//=> 입력스트림에 엔터(Enter)가 입력될 경우 스레드 다시 실행
			//=> 입력스트림 또는 출력스트림 관련 메소드 IOException 발생(입력스트림에 문제있을때)
			//=> IOException은 일반 예외이므로 반드시 예외처리
			readByte=System.in.read(); //에러 발생 시 throws로 떠넘기기(거의 발생하지 않기 때문)
