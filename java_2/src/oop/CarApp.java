...
//=> 참조변수에 저장된 객체는 .연산자를 사용하여 필드 또는 메소드에 접근하여 프로그램 작성


//Car클래스를 객체로 생성하여 작성된 프로그램
public class CarApp {
	public static void main(String[] args) {
		...
		//해결법) 클래스 작성 시 필드를 은닉화 처리하여 선언 - 은닉화 선언된 필드에 접근할 경우 에러 발생
		//carOne.modelName="싼타페"; //객체의 필드값 변경 - 필드가 은닉화 처리되어 에러 발생
		carOne.setModelName("싼타페");
		//carOne.engineStatus=false;
		//carOne.currentSpeed=0;
		
		//System.out.println("첫 번째 자동차 모델명 = "+carOne.modelName); //객체의 필드값 사용
		//객체의 필드값을 반환하는 Getter 메소드 호출 - 반환받은 필드값 사용(출력)
		System.out.println("첫 번째 자동차 모델명 = "+carOne.getModelName()); //객체의 필드값 사용
		System.out.println("첫 번째 자동차 엔진상태 = "+carOne.isEngineStatus());
		System.out.println("첫 번째 자동차 모델명 = "+carOne.getCurrentSpeed());
		...
		System.out.println("두 번째 자동차 모델명 = "+carTwo.getModelName()); //객체의 필드값 사용
		System.out.println("두 번째 자동차 엔진상태 = "+carTwo.isEngineStatus());
		System.out.println("두 번째 자동차 모델명 = "+carTwo.getCurrentSpeed());

		System.out.println("===============================================================");
		
		//참조변수.메소드(값,값...) : 참조변수에 저장된 객체가 .연산자를 사용하여 메소드 호출
		carOne.startEngine();
		carOne.speedUp(50);
		carOne.speedUp(30);
		carOne.speedDown(40);
