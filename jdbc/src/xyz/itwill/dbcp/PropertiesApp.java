...
public class PropertiesApp {
	...
	/*
		...
		*/
		...
		/*
			Properties.load(InputStream in) : 입력스트림으로 Properties 파일을 제공받아 파일에 저장된 모든 이름과 값으로 Properties 객체에 엔트리를 추가하는 메소드
		*/
		properties.load(in);
		
		/*
			properties.get(String key) : Properties 객체에 저장된 엔트리에서 맵키(Map Key)를 전달받아 맵값(Map Value)을 반환하는 메소드
			  => 맵값(Map Value)은 Object 객체로 반환되므로 반드시 명시적 객체 형변환 후 사용 가능
		*/
