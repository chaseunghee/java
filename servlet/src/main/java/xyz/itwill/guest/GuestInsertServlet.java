...
/*
	방명록 게시글을 전달받아 GUEST 테이블에 삽입하고 방명록 게시글 목록페이지(/guest/list.itwill)로 이동하기 위한 URL 주소를 클라이언트에게 전달하여 응답하는 서블릿
...
public class GuestInsertServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//리퀘스트 메세지 몸체부(post방식으로 요청할 때만 사용)에 저장되어 전달되는 값에 대한 캐릭터셋 변경
		...
		GuestDAO.getDao().inserGuest(guest);
