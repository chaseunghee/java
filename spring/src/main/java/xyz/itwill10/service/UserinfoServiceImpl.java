...
//[순서-12] - UserinfoServiceImpl 클래스 생성
...
/* [순서-13] - 주석
	...
	=> BCrypt.gensalt(int log_bounds) : 매개변수로 첨가물(Salt - String)의 길이를 전달받아 첨가물을 생성하여 반환하는 메소드 
	  - 매개변수에 길이를 전달하지 않으면 자동으로 [10]으로 설정
	3.BCrypt.checkpw(String plaintext, String hashed) 메소드로 암호화된 비밀번호를 비교하여 실행결과를 반환받아 처리 
	=> 매개변수로 일반 문자열과 암호화된 문자열을 전달받아 비교하여 다른 경우 [false]를 반환하고 같은 경우 [true]를 반환하는 메소드
*/
...
public class UserinfoServiceImpl implements UserinfoService {
	...
	@Transactional(rollbackFor = Exception.class) //[순서-16] - 예외떨어지면 rollback처리하기 위해 @Transactiional 작성
	@Override
	public void addUserinfo(Userinfo userinfo) throws ExistsUserinfoException {
		...
		if(userinfoDAO.selectUserinfo(userinfo.getUserid()) != null) {
			//예외를 명확하기 구분하고 예외처리시 필요한 값을 제공하기 위해 직접 생성한 예외 클래스를 사용하여 인위적인 예외 발생
			throw new ExistsUserinfoException("이미 사용중인 아이디를 입력 하였습니다.", userinfo);
		}
		
		// [순서-15] - 매개변수로 전달받은 회원정보의 비밀번호를 암호화 처리하여 필드값으로 다시 저장
	...
	*/
	...
	public Userinfo loginAuth(Userinfo userinfo) throws LoginAuthFailException {
		...
		//매개변수로 전달받은 회원정보의 비밀번호와 검색된 회원정보의 비밀번호를 비교하여 같지 않은 경우 - 비밀번호 인증 실패
