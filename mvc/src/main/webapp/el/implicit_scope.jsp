...
<%
	/*
		...
		Scope 객체 속성값의 사용범위 : page Scope, Request Scope, Session Scope, Application Scope
	*/
	...
	/*
		...
		=> 같은 JSP 내장객체에 동일한 속성명을 사용하여 속성값을 저장하면 기존 속성값 대신 새로운 속성값이 저장 - 변경 처리
	*/
...
<body>
	...
	<ul>
		<%-- 
			...
			=> Scope 객체에 동일한 이름의 속성명을 사용하여 속성값을 저장한 경우 먼저 검색된 Scope 객체의 속성값을 제공받아 출력 처리 - Scope 객체의 속성명은 다른게 설정하는 것을 권장 
		--%>
		<%--  ${name }에서 name은 Page Scope에서 찾을 수 있음 -> 위에 name이라는 이름으로 홍길동 저장되어있음 => 그럼 모든 li의 name이 홍길동
		...
		<%-- 
			Scope 객체에 동일한 이름의 속성명을 사용하여 속성값을 저장한 경우 EL 내장객체를 사용하여 스코프 객체를 구분하여 속성값을 제공받아 출력 처리
