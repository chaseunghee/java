...
public final class ConnectionPool {
	// Singleton - 이유 : 객체의 인스턴스가 오직 1개만 생성
	private static ConnectionPool instance;

	//컨넥션2개(free,used) in Vector 
	// Storage for the unused connections. - Vector == ArrayList 
	private Vector<Connection> free; //free - 사용하지 않는 컬넥션 저장

	// Storage for the allocated connections.
	private Vector<Connection> used; //used - 사용하는 컬넥션 저장

	// Connection information. - 컬넥션 정보를 저장하는 필드
	...
	private ConnectionPool() throws SQLException {
		...
		free = new Vector<Connection>(initialCons); //Vector 객체 요소 안에 initialCons 개수(2) 
	...
	private void loadConf() {
		...
		this.driver=(String)p.get("driver"); //properties 값들을 생성자를 통해 필드에 저장
	 ...
	 * @see java.sql.DriverManager
	 **/
	public synchronized Connection getConnection(boolean block, long timeout) throws SQLException { //getConnection : 미리만들어진 컨넥션 제공받음

		if (free.isEmpty()) {

			// None left, do we create more?
			if (maxCons <= 0 || numCons < maxCons) { //컨넥션 3개까지 
				addConnection();
			} else if (block) {
				try {
