...
/*
	DAO(Data Access Object) 클래스 : 테이블에 행 삽입, 삭제, 변경, 검색하는 SQL 명령을 전달하여 실행하고 실행결과 Java 객체(값)을 매핑하여 반환하는 기능을 제공하는 클래스
	=> 싱클톤 클래스(프로그램에 객체를 하나만 생성하여 제공하는 클래스)로 작성하는 것을 권장
*/
...
public class StudentDAO extends JdbcDAO{
	...
	public List<StudentDTO> selectStudentList(){
		...
		try {
			...
			while(rs.next()) {
				/*
					resultSet 커서 위치에 행을 DTO 객체로 표현
					=> resultSet 커서 위치의 행에 대한 컬럼값은 DTO 객체의 필드에 매핑하여 저장
				*/
				StudentDTO student=new StudentDTO();
				student.setNo(rs.getInt("no")); //no값으로 필드값 변경
