...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 
	가상의 클라이언트(브라우저)를 이용하여 다른 서버의 웹프로그램을 요청하고 실행결과를 제공받아 클라이언트에게 그대로 전달하여 응답하는 JSP 문서 
...
//요청 웹프로그램의 URL 주소 저장
	...
	*/
	GetMethod method=new GetMethod(url); //GET 방식
	...
	try {
		...
		*/
		...
		if(statusCode==HttpServletResponse.SC_OK) {//정상적인 실행결과를 응답받은 경우 - 상태코드 : 200
			/*
				Method.getResponseBodyAsString() : 요청에 대한 실행결과가 저장된 리스폰즈 메세지 몸체부의 결과값을 문자열로 반환하는 메소드
		...
		} finally {//catch가 아니라 finally 쓰는 이유
