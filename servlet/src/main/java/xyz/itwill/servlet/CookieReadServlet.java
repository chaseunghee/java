...
public class CookieReadServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		Cookie[] cookies=request.getCookies(); //클라이언트로부터 전달받은 쿠키를 배열로 받음
		...
		out.println("</head>");
		out.println("<body>");
		out.println("<h1>쿠키(Cookie) 읽기</h1>");
		out.println("<hr>");
		if(cookies == null) { //클라이언트로부터 전달받은 쿠키가 없는 경우
			...
			//Cookie 객체배열에 있는 요소(Cookie 객체)를 차례대로 제공받아 변수에 저장하여 사용하기 위한 반복문
			for(Cookie cookie : cookies) {
				//클라이언트로부터 전달받은 쿠키에서 필요로 하는 쿠키값을 반환받아 저장 => 이름을 이용해 쿠키값을 반환받아 저장하는 코드 작성
				
				//Cookie.getName() : Cookie 객체에 저장된 쿠키명을 반환하는 메소드
				if(cookie.getName().equals("id")) { //id라는 쿠키가 있는 경우
			...
			if(!id.equals("")) {//쿠키값이 있는 경우(null String이 아닐 경우)
