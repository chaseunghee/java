...
<%
	...
	/*
		사용자로부터 입력받아 전달된 학생번호가 STUDENT 테이블에 저장된 기존 학생정보의 학생번호와 중복될 경우 [insertFormStudent.jsp] 문서로 이동할 수 있는 URL 주소를 클라이언트에게 전달
		=> null 반환 : 학생정보 미검색 -> 학생번호 미중복
		=> StudentDTO 객체 반환 : 학생정보 검색 -> 학생번호 중복
	*/
	if(StudentDAO.getDAO().selectStudent(no)!=null) {//학생번호가 중복된 경우
		session.setAttribute("message", "이미 사용중인 학생번호를 입력 하였습니다. 다시 입력해 주세요.");
		session.setAttribute("student", student); //student : 
		response.sendRedirect("insertFormStudent.jsp");//클라이언트에게 URL 주소 전달
		return;
	}
