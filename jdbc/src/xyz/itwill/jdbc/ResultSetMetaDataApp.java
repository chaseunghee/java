...
public class ResultSetMetaDataApp {
	public static void main(String[] args) throws SQLException{
		...
		String sql="select * from student order by no"; // -> 결과값 : 컬럼의 개수 : 5
		//String sql="select no,name from student order by no"; // -> 결과값 : 컬럼의 개수 : 2
		...
		//ResultSet.getmetaData() : 검색행(ResultSet 객체)에 대한 부가적인 정보가 저장된  ResultSetMetaDate 객체를 반환하는 메소드
		...
		for(int i=1; i<=columnCount; i++) {
			...
			//ResultSetmetaDat.isNullable(int columnIdex) : 첨자 위치(columnIndex)의 컬럼에 대한 NULL 허용 유무값(0 또는 1)을 반환하는 메소드
			...
			//ResultSetmetaData.getColumnDisplaySize(int columnIndex) : 첨자 위치(columnIndex)의 컬럼에 대한 출력 크기를 반환하는 메소드
			...
			System.out.println("컬럼의 크기 = "+columnDisplaySize);
