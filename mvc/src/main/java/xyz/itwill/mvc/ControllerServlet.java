...
/*
	컨트롤러(Controller - Servlet) : 클라이언트의 모든 요청을 받아 모델(Model - Class) 역활의 객체로 요청 처리 메소드를 호출하여 클라이언트의 요청을 처리하고 
									처리결과를 뷰(View - JSP)로 전달하여 응답되도록 프로그램의 흐름을 제공하는 웹프로그램
	...
	=> @WebServlet 어노테이션 대신 [web.xml] 파일의 엘리먼트를 사용하여 클래스를 웹프로그램으로 등록하고 URL 매핑 처리 가능
...
public class ControllerServlet extends HttpServlet {
	...
	/* ◆ 대신 ★-1
		...
		=> 하나의 엔트리는 요청정보(Command - (Key - String))와 모델객체(Value - Action)를 이용하여 Map 객체의 필드 생성
	...
	*/
	...
	//★-2 init - 클라이언트가 서블릿(컨트롤러)를 최초로 요청할 경우 서블릿 클래스를 객체로 생성한 후 가장 먼저 한 번만 호출되는 메소드 - 생성자 대신 초기화 작업을 실행하기 위한 메소드
	@Override
	 ...
	 */
		...
		*/
		...
		//ServletConfig.getInitParameter(String name) : [web.xml] 파일에서 init-param 엘리먼트로 제공된 값을 읽어와 반환하는 메소드
		...
		//Properties 객체에 저장된 모든 엔트리의 이름(Key)이 저장된 Set 객체를 반환받아 for 구문을 이용하여 반복 처리
		for(Object key : properties.keySet()) {
			...
			try {
				/*
					...
					리플렉션(Reflection) : 프로그램의 명령 실행시 Class 객체(Clazz)로 객체를 생성하여 객체의 필드 또는 메소드에 접근하도록 제공하는 기능
					
					Class.forName(String className) : 매개변수로 전달받은 문자열로 표현된 클래스를 읽어 메모리에 저장하고 Class 객체(Clazz)를 반환하는 메소드
					=> ClassNotFoundException 발생
					
					Class.getDeclaredConstructor() : Class 객체의 생성자가 저장된 Constructor 객체를 반환하는 메소드
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		/*
			3. 모델(Model) 객체를 이용하여 요청 처리 메소드를 호출해 클라이언트의 요청을 처리하고 뷰(View) 관련 정보를 반환받아 저장
		...
		*/ 
		...
		if(command.equals("/loginform.do")) {
			action=new LoginFormModel(); //xyz.itwill.mvc >> LoginFormModel >> webapp >> user_login
		} else if(command.equals("/login.do")) {
			action=new LoginModel(); //xyz.itwill.mvc >> LoginModel >> 
		} else if(command.equals("/logout.do")) {
			action=new LogoutModel(); //xyz.itwill.mvc >> LogoutModel >> 
		} else if(command.equals("/writeform.do")) {
			action=new writeformModel(); //xyz.itwill.mvc >> writeformModel >> 
		} else if(command.equals("/write.do")) {
			action=new writeModel(); //xyz.itwill.mvc >> writeModel >>  
		} else if(command.equals("/list.do")) {
			action=new ListModel(); //xyz.itwill.mvc >> ListModel >> user_list.jsp 
		} else if(command.equals("/view.do")) {
			action=new ViewModel();//xyz.itwill.mvc >> ViewModel
		} else if(command.equals("/modifyform.do")) {
			action=new ModifyFormModel();//xyz.itwill.mvc >> ModifyModel >> user_modify.jsp
		} else if(command.equals("/modify.do")) {
			action=new ModifyModel();
		} else if(command.equals("/remove.do")) {
			action=new RemoveModel();
		} else if(command.equals("/error.do")) {
			action=new ErrorModel();//xyz.itwill.mvc >> ErrorModel.java >> webapp >> user_error.jsp
		...
		*/
		...
		/*
			인터페이스 참조변수로 추상메소드를 호출하면 참조변수에 저장된 모델 객체의 요청 처리 메소드를 호출하고 뷰 관련 정보를 반환받아 저장 -  오버라이드의 의한 다형성
		*/
		ActionForward actionForward=action.execute(request, response);
		
		/*
			4. 응답 관련 정보가 저장된 ActionForward 객체를 이용하여 응답 처리 -true : forward / false : redirect
		*/
