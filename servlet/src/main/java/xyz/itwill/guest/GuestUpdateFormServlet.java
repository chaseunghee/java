...
/*
	글번호를 전달받아 GUEST 테이블에 저장된 방명록 게시글을 검색하여 입력태그의 초기값으로 설정하고 사용자로부터 방명록 게시글(변경값)을 입력받기 위한 서블릿
	=> [글변경] 태그를 클릭한 경우 방명록 게시글 변경페이지(/guest/modify.itwill)로 이동 - 입력값
	=> [글목록] 태그를 클릭한 경우 방명록 게시글 
*/
...
public class GuestUpdateFormServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//글번호를 전달받아 GUEST 테이블에 저장된 해당 글번호의 방명록 게시글을 검색하여 DTO 객체를 반환하는 DAO 클래스의 메소드 호출
