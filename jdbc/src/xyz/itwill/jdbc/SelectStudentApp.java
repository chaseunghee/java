...
public class SelectStudentApp {
	public static void main(String[] args) {
		...
		try {
			...
			String sql="select * from student order by no"; //오라클에서 정렬된 순서대로 검색 (다중행)
		  //String sql="select no,name,address,phone,birthday from student order by no";
			...
			*/
			...
			/*
			 	...
			 	*ResultSet.next() : ResultSet 커서를 다음행으로 이동하는 메소드
			 	  => false 반환 : ResultSet 커서 위치에 처리행이 없는 경우의 반환값 - ResultSet 커서가 EOF() 영역에 위치
