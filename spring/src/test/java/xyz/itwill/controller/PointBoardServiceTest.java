...
/* [순서-17]
	...
	=> Spring Bean Configuration File의 AOP 설정을 이용하여 TaransactionManager 객체를 사용하여 트렌젝션 처리 할 수도 있지만 
		@Tranactional 어노테이션을 사용하여 트렌젝션 처리 가능
	
	TaransactionManager 객체를 이용하여 트렌젝션 처리 기능을 제공받을 메소드에 @Tranactional 어노테이션 사용하면 메소드의 명령 실행시 예외(Exception)가 발생된 경우 자동으로 롤백 처리
	=> @Tranactional 어노테이션을 사용하기 위해 Spring Bean Configuration File(root-context.xml)에 tx 네임스페이스를 추가하여 spring-tx.xsd 파일을 제공받아 
		annotaion-driven 엘리먼트를 반드시 설정
	=> 테스트 클래스의 테스트 메소드에 @Tranactional 어노테이션을 사용하면 테스트 메소드의 명령 실행 후 무조건 롤백 처리
*/
...
public class PointBoardServiceTest {
	...
	public void test1() throws Exception {
		...
		//PointBoard board=PointBoard.builder().writer("xyz789").subject("테스트").build(); - 일부러 예외 발생시키기 위한 코드 
		...
		/*
			...
			-문제점)예외 발생전에 실행된 게시글 삽입에 대한 SQL 명령은 이미 DBMS 서버에 전달되어 실행된 상태이므로 POINTBOARD 테이블에는 비정상적인 게시글 저장
